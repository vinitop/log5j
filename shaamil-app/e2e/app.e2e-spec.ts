...
describe('Starting tests for shaamil-app', function() {
  ...
  it('website title should be shaamil-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('shaamil-app');
    })
  });

  it('network-name should be shaamil-network@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('shaamil-network@0.0.1.bna');
    });
  });

  it('navbar-brand should be shaamil-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('shaamil-app');
    });
  });
